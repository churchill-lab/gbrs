...
def bam2emase(**kwargs):
    ...
    if lidfile is None:
        ...
        if os.path.exists(lidfile2chk):
            ...
            print("[gbrs::bam2emase] Cannot find a locus id file.", file=sys.stderr)
            return 2
    outfile = kwargs.get("outfile")
    if outfile is None:
        outfile = (
            "gbrs.bam2emased." + os.path.splitext(os.path.basename(alnfile))[0] + ".h5"
        )
    ...
    alignmat_factory.prepare(
        haplotypes, loci, delim=delim, outdir=os.path.dirname(outfile)
    )
...
def intersect(**kwargs):
    ...
    for f in flist[1:]:
        ...
        if np.all(alnmat.rname == alnmat_next.rname):
            ...
            print(
                "[gbrs::intersect] The read ID's are not compatible.", file=sys.stderr
            )
            return 2
...
def compress(**kwargs):
    ...
    for alnfile in flist:
        ...
        for curind in range(alnmat_rd.num_reads):
            ec_key = []
            for h in range(alnmat_rd.num_haplotypes):
                i0 = alnmat_rd.data[h].indptr[curind]
                i1 = alnmat_rd.data[h].indptr[curind + 1]
                ec_key.append(
                    ",".join(map(str, sorted(alnmat_rd.data[h].indices[i0:i1])))
                )
            ec[":".join(ec_key)] += alnmat_rd.count[curind]
    ec = dict(ec)
    num_ecs = len(ec)
    alnmat_ec = emase.AlignmentPropertyMatrix(
        shape=(alnmat_rd.num_loci, alnmat_rd.num_haplotypes, num_ecs)
    )
...
def quantify(**kwargs):
    ...
    if grpfile is None:
        ...
        if os.path.exists(grpfile2chk):
            ...
            print(
                "[gbrs::quantify] A group file is not given. Group-level results will not be reported.",
                file=sys.stderr,
            )
    ...
    if lenfile is None:
        ...
        if os.path.exists(lenfile2chk):
            ...
            print(
                "[gbrs::quantify] A length file is not given. Transcript length adjustment will *not* be performed.",
                file=sys.stderr,
            )
    ...
    report_group_counts = (
        grpfile is not None
    )  # If grpfile exist, always report groupwise results too
    ...
    if gtypefile is not None:  # Genotype calls are at the gene level
        ...
        gtcall_g = None
        gtcall_t = None
    print(f"[gbrs::quantify] Outbase={outbase}")
    # Run emase
    ...
    em_factory.report_depths(
        filename="%s.isoforms.tpm" % outbase, tpm=True, notes=gtcall_t
    )
    em_factory.report_read_counts(
        filename="%s.isoforms.expected_read_counts" % outbase, notes=gtcall_t
    )
    if report_posterior:
        em_factory.export_posterior_probability(filename="%s.posterior.h5" % outbase)
    if report_group_counts:
        em_factory.report_depths(
            filename="%s.genes.tpm" % outbase, tpm=True, grp_wise=True, notes=gtcall_g
        )
        em_factory.report_read_counts(
            filename="%s.genes.expected_read_counts" % outbase,
            grp_wise=True,
            notes=gtcall_g,
        )
