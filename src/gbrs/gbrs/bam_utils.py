...
class BAMGenerator:
    ...
    def __post_init__(self):
        ...
        if self.haplotypes:
            self.haplotype_list = [
                h.strip() for h in self.haplotypes.split(',')
            ]
            if len(self.haplotype_list) != self.num_haplotypes:
                raise ValueError(
                    f'Number of haplotypes in --haplotypes ({len(self.haplotype_list)}) '
                    f'must match -h ({self.num_haplotypes})'
                )
        else:
        ...
        self.loci = [
            f'{self.prefix_locus}{str(i).zfill(11)}'
            for i in range(1, self.num_loci + 1)
        ]

        # generate read names
        self.read_names = [
            f'{self.prefix_read}{str(i).zfill(12)}'
            for i in range(1, self.num_read_names + 1)
        ]
        ...
        max_combinations = (
            self.num_loci * self.num_haplotypes * self.num_read_names
        )
        if self.alignments_r1 > max_combinations:
            raise ValueError(
                f'alignments_r1 ({self.alignments_r1}) cannot exceed '
                f'loci×haplotypes×readnames ({max_combinations})'
            )
        if self.alignments_r2 > max_combinations:
            raise ValueError(
                f'alignments_r2 ({self.alignments_r2}) cannot exceed '
                f'loci×haplotypes×readnames ({max_combinations})'
            )
        ...
        self.selected_combinations_r1 = self._select_combinations(
            self.alignments_r1
        )
        ...
        if remaining_slots > 0:
            ...
            if self.coverage_distribution == 'uniform':
                ...
                selected_indices = np.random.choice(
                    ...
                    size=min(remaining_slots, len(remaining_combinations)),
                    replace=False,
                    p=weights,
                )
                ...
                raise ValueError(
                    f'Unknown coverage distribution: {self.coverage_distribution}'
                )
    ...
    def _generate_read_sequence(
        self, ref_sequence: str, start_pos: int
    ) -> Tuple[str, int, List]:
    ...
    def _generate_quality_scores(self) -> str:
        ...
        for i in range(self.read_length):
            # Qqality degrades toward the end of the read
    ...
    def generate_bam(
        ...
        for locus in self.loci:
            for haplotype in self.haplotype_list:
                ...
                header['SQ'].append({'LN': ref_length, 'SN': ref_name})
        ...
        for read_name, locus, haplotype in self.selected_combinations_r1:
            ...
            (
                ...
                num_mismatches,
                cigar_ops,
            ) = self._generate_read_sequence(ref_sequence, start_pos)
            ...
            read_entry = {
                ...
                'tags': [
                    ...
                    (
                        'MD',
                        self._generate_md_tag(
                            ref_sequence[
                                start_pos : start_pos + len(read_sequence)
                            ],
                            read_sequence,
                        ),
                    ),
    ...
    def _write_single_bam(
        self, header: dict, reads: List[dict], output_file: str
    ):
    ...
    def _create_segment(
        self, read_data: dict, ref_name_to_id: dict
    ...
    def generate_locus_file(self, output_file: str) -> None:
