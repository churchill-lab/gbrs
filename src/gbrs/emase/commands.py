...
def bam2emase_paired(
    alignment_files: Annotated[Path, typer.Option(
...
def combine(
    ...
    comp_lib: Annotated[str, typer.Option(
        '-c', '--comp-lib', help='compression library to use'
    )] = 'zlib',
    verbose: Annotated[int, typer.Option(
        '-v', '--verbose', count=True,
        help="specify multiple times for more verbose output"
    )] = 0
) -> None:
    logger = utils.configure_logging('gbrs', verbose)
...
def count_alignments(
    ...
    group_file: Annotated[Path, typer.Option(
        ...
        resolve_path=True, help="gene ID to isoform ID mapping info (tsv)"
    )],
    outbase: Annotated[str, typer.Option(
        '-o', '--outbase', help='basename of all the generated output files'
    )] = 'emase',
    verbose: Annotated[int, typer.Option(
        '-v', '--verbose', count=True,
        help="specify multiple times for more verbose output"
...
def count_shared_multireads_pairwise(
    alignment_file: Annotated[Path, typer.Option(
        '-i', '--alignment-file', exists=True, dir_okay=False,
        resolve_path=True, help="EMASE file"
    )],
    group_file: Annotated[Path, typer.Option(
        '-g', '--group-file', exists=True, dir_okay=False,
        resolve_path=True, help="gene ID to isoform ID mapping info (tsv)"
...
def create_hybrid(
    fasta_list: Annotated[list[Path | str], typer.Option(
        '-F', '--target-files', exists=False, dir_okay=False,
        resolve_path=True, help='Fasta file to parse, can separate files by "," or have multiple -i'
    )],
    haplotypes: Annotated[list[str], typer.Option(
        '-s', '--suffices', help='haplotype, either one per -h option, '
        'i.e. -h A -h B -h C, or a shortcut -h A,B,C'
    )],
    output_file: Annotated[Path, typer.Option(
        '-o', '--output', exists=False, dir_okay=False,
        writable=True, resolve_path=True
    )] = 'gbrs.hybridized.targets.fa',
...
def get_common_alignments(
    ...
    output_file: Annotated[Path, typer.Option(
        '-o', '--output', exists=False, dir_okay=False,
        writable=True, resolve_path=True, help="EMASE file with unique reads"
    )] = None,
    comp_lib: Annotated[str, typer.Option(
        '-c', '--comp-lib', help='compression library to use'
    )] = 'zlib',
...
def pull_out_unique_reads(
    alignment_file: Annotated[Path, typer.Option(
        '-i', '--alignment-file', exists=True, dir_okay=False,
        resolve_path=True, help="Alignment profile (pseudo-alignment) in EMASE format"
    )],
    output_file: Annotated[Path, typer.Option(
        '-o', '--output', exists=False, dir_okay=False,
        writable=True, resolve_path=True, help="EMASE file with unique reads"
    ...
    shallow: Annotated[bool, typer.Option(
        '-s', '--shallow', help='return shallow EMASE file'
    )] = False,
    ignore_alleles: Annotated[bool, typer.Option(
        '-a', '--ignore-alleles', help='do not require allele level uniqueness'
    )] = False,
    verbose: Annotated[int, typer.Option(
        '-v', '--verbose', count=True,
        help="specify multiple times for more verbose output"
...
def prepare(
    genome_file: Annotated[list[Path], typer.Option(
        '-G', '--genome-file', exists=False, dir_okay=False,
        resolve_path=True, help='Genome files, can separate files by "," or have multiple -G'
    )],
    haplotypes: Annotated[list[str], typer.Option(
        '-s', '--haplotype-char', help='haplotype, either one per -h option, '
        'i.e. -h A -h B -h C, or a shortcut -h A,B,C'
    )] = None,
    gtf_file: Annotated[list[Path], typer.Option(
        '-g', '--gtf-file', exists=False, dir_okay=False,
        resolve_path=True, help='Gene Annotation File files, can separate files by "," or have multiple -G'
    )] = None,
    out_dir: Annotated[str, typer.Option(
        '-o', '--out_dir', help='Output folder to store results (default: the current working directory)'
    )] = None,
    save_g2tmap: Annotated[bool, typer.Option(
        '-m', '--save-g2tmap', help='saves gene id to transcript id list in a tab-delimited text file'
    )] = False,
    save_dbs: Annotated[bool, typer.Option(
        '-d', '--save-dbs', help='save dbs'
    )] = False,
    no_bowtie_index: Annotated[bool, typer.Option(
        '-m', '--no-bowtie-index', help='skips building bowtie index'
    )] = False,
    verbose: Annotated[int, typer.Option(
        '-v', '--verbose', count=True,
        help="specify multiple times for more verbose output"
...
def run(
    ...
    group_file: Annotated[Path, typer.Option(
        '-g', '--group-file', exists=True, dir_okay=False,
        resolve_path=True, help='tab delimited file of gene to transcript mapping'
    )] = None,
    length_file: Annotated[Path, typer.Option(
        '-L', '--length-file', exists=True, dir_okay=False,
        resolve_path=True, help='tab delimited file of locus(transcript) and length'
    )] = None,
    outbase: Annotated[str, typer.Option(
        '-o', '--outbase', help='basename of all the generated output files'
    )] = 'emase',
    multiread_model: Annotated[int, typer.Option(
        '-M', '--multiread-model', help='emase model (default: 4)'
    )] = 4,
    pseudocount: Annotated[float, typer.Option(
        '-p', '--pseudocount', help='prior read count (default: 0.0)'
    )] = 0.0,
    read_length: Annotated[int, typer.Option(
        '-l', '--read-length', help='specify read length'
    )] = 100,
    max_iters: Annotated[int, typer.Option(
        '-m', '--max-iters', help='maximum iterations for EM iteration'
    )] = 999,
    tolerance: Annotated[float, typer.Option(
        '-t', '--tolerance', help='tolerance for EM termination (default: 0.0001 in TPM)'
    )] = 0.0001,
    report_alignment_counts: Annotated[bool, typer.Option(
        '-c', '--report-alignment-counts', help='whether to report alignment counts'
    )] = False,
    report_posterior: Annotated[bool, typer.Option(
        '-w', '--report-posterior', help='whether to report posterior probabilities'
    )] = False,
    verbose: Annotated[int, typer.Option(
        '-v', '--verbose', count=True,
        help="specify multiple times for more verbose output"
    ...
    try:
        if multiread_model not in (1, 2, 3, 4):
            raise typer.Abort(
                '-M, --multiread-model must be one of 1, 2, 3, or 4'
            )
